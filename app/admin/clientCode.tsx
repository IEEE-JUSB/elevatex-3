...
import { AdminDataType, closeRegistration, getAdminData } from './adminUtils';
...
function ClientCode() {
  ...
  return (
    <div className='min-h-screen bg-blue-400 p-6 flex flex-col items-center'>
      ...
      {adminData?.eventRegistrations.map((e) => <div key={e.name} className={`p-2 hover:bg-gray-800/30 transition-colors duration-200 grid grid-cols-3 place-items-center w-4/5 rounded-lg`}>
        <p>{e.name}</p>
        <p>{e.count}</p>
